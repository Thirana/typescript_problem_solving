...
class OrderBook {
  ...
  addOrder(order: Order): void {
    ...
    const ordersToMatch =
      order.type === "buy" ? this.sellOrders : this.buyOrders;
    ...
    while (remainingQty > 0 && ordersToMatch.length > 0) {
      ...
      //log
      console.log(
        `Matched ${order.id} with ${bestMatch.id} for ${matchQty} units at price ${matchPrice}`,
      );
    ...
    if (remainingQty > 0) {
      const newOrder = { ...order, quantity: remainingQty };
      if (isBuy) {
